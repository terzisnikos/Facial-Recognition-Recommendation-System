...
import * as dataFormat from 'pages/CRUD/Faces/table/FacesDataFormatters';
...
const FacesTable = () => {
  ...
  const loading = useTypedSelector((store) => store.faces.list.loading);
  const rows = useTypedSelector((store) => store.faces.list.rows);
  const count = useTypedSelector((store) => store.faces.list.count);
  const modalOpen = useTypedSelector((store) => store.faces.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.faces.list.idToDelete);
  ...
  const actionFormatter = (cell: string) => {
    return (
      <div>
        {null && (
          <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/faces/${cell}`))}>
            View
          </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/faces/${cell}/edit`)}>
          Edit
        </Button>
        &nbsp;&nbsp;
