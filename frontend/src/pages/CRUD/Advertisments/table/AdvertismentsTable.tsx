...
import * as dataFormat from 'pages/CRUD/Advertisments/table/AdvertismentsDataFormatters';

// eslint-disable-next-line
import * as usersDataFormat from 'pages/CRUD/Users/table/UsersDataFormatters';

import actions from 'store/actions/advertisments/advertismentsListActions';
...
const AdvertismentsTable = () => {
  ...
  const loading = useTypedSelector((store) => store.advertisments.list.loading);
  const rows = useTypedSelector((store) => store.advertisments.list.rows);
  const count = useTypedSelector((store) => store.advertisments.list.count);
  const modalOpen = useTypedSelector((store) => store.advertisments.list.modalOpen);
  const idToDelete = useTypedSelector((store) => store.advertisments.list.idToDelete);
  ...
  const actionFormatter = (cell: string) => {
    return (
      <div>
        {null && (
          <Button color="default" size="xs" onClick={() => dispatch(push(`/admin/advertisments/${cell}`))}>
            View
          </Button>
        )}
        <Button color="info" size="xs" onClick={() => history.push(`/admin/advertisments/${cell}/edit`)}>
          Edit
        </Button>
        &nbsp;&nbsp;
        <Button color="danger" size="xs" onClick={(event: React.MouseEvent) => openModal(event, cell)}>
          Delete
        </Button>
      </div>
    );
  ...
  return (
    <div>
      <Widget title={<h4>Advertisments</h4>} collapse close>
        <Link to="/admin/advertisments/new">
          <button
